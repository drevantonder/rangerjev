...
function stateFor(units, context) {
    const unitsValue = units.map((unit) => ({
        id: unit.id,
        path: unit.path,
        source: unit.source,
    ...
    const state = { units: unitsValue };
    if (context !== undefined)
        state["context"] = context;
    return state;
}
function plannedQuestions(units, questions) {
    void units;
    const planned = [];
...
function questionPayload(unitIndex, question) {
    const instructions = {
...
export function planBatches(units, questions, context) {
...
function checkAnswer(question, answer) {
    ...
    if (question.kind === "choice") {
        ...
        const out = { type: "choice", choice: answer.choice };
    ...
    const out = { type: "score", score: answer.score };
    if (answer.probabilities !== undefined && typeof answer.probabilities === "object") {
        out.probabilities = answer.probabilities;
    }
    if (typeof answer.confidence === "number")
...
async function evaluateBatch(state, batch, evaluator, results, unanswered, units, usage) {
    const payload = {};
...
export function summarize(units, results, questions) {
    const summary = {};
    for (const question of questions) {
        if (question.kind === "boolean") {
            let sum = 0;
