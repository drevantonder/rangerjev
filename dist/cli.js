...
import { collectFiles, loadUnitFinder, splitBy, splitCustom, } from "./splitter.js";
...
function parseCount(raw, flag, min) {
    if (raw === undefined)
        return undefined;
...
function parseArgs(args) {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (arg === "--help") {
            ...
            options.depth = parseCount(next(), "--depth", 0) ?? 3;
        else if (arg === "--unit-finder")
            options.unitFinder = next();
        else if (arg === "--questions")
            options.questionsPath = next();
        else if (arg === "--boolean")
            options.inline.booleans.push(next());
        else if (arg === "--choice")
            options.inline.choices.push(next());
        else if (arg === "--choices")
            options.inline.choicesFor.push(next());
        else if (arg === "--score")
            options.inline.scores.push(next());
        else if (arg === "--levels")
            options.inline.levelsFor.push(next());
        else if (arg === "--context")
            options.contextPath = next();
        else if (arg === "--ext") {
            options.extraExts.push(...next()
                .split(",")
                .map((ext) => ext.trim())
                .filter((ext) => ext !== ""));
        }
        else if (arg === "--max-units")
            options.maxUnits = parseCount(next(), "--max-units", 1);
        else if (arg === "--format") {
            const value = next();
...
function formatText(report) {
    ...
    for (const unit of report.units) {
        for (const [id, answer] of Object.entries(unit.answers)) {
            if (answer.type === "boolean")
                lines.push(`${answer.probability.toFixed(3)}  ${unit.path}  ${id}`);
            else if (answer.type === "choice")
                lines.push(`${answer.choice}  ${unit.path}  ${id}`);
            else
                lines.push(`${answer.score.toFixed(2)}  ${unit.path}  ${id}`);
        }
        if (Object.keys(unit.answers).length === 0)
            lines.push(`---  ${unit.path}  unanswered`);
    }
    for (const [id, entry] of Object.entries(report.summary)) {
        if (entry.type === "boolean") {
            ...
            lines.push(`# ${id}: n=${entry.n} mean=${entry.mean.toFixed(2)} min=${entry.min.toFixed(2)} max=${entry.max.toFixed(2)}`);
        }
    }
    lines.push(`${report.coverage.unitsAsked}/${report.coverage.unitsEnumerated} units asked; ` +
...
export async function runCli(args, dependencies = {}) {
    ...
    try {
        ...
        const fromFile = options.questionsPath === undefined
            ? []
    ...
    if (options.maxUnits !== undefined)
        units = units.slice(0, options.maxUnits);
    if (units.length === 0)
        return fail(stderr, "no units to ask about");
    let context;
    if (options.contextPath !== undefined) {
        try {
            ...
            return fail(stderr, `cannot read --context file: ${error.message}`);
        }
    }
    if (options.dryRun) {
        const planned = units.length * questions.length;
        stderr(`rangerjev: dry run: ${units.length} units x ${questions.length} questions = ${planned} planned questions, 0 live requests\n`);
