...
function splitIdValue(raw, flag) {
    const index = raw.indexOf("=");
    if (index <= 0)
        throw new Error(`${flag} must look like id=text, got: ${raw}`);
...
function splitList(raw, flag) {
    ...
    const items = value
        ...
        .filter((item) => item !== "");
    if (items.length === 0)
        throw new Error(`${flag} needs at least one value: ${raw}`);
...
export function parseInline(inline) {
    const out = [];
    const seen = new Set();
    const claim = (id, flag) => {
        if (seen.has(id))
            throw new Error(`duplicate question id: ${id} (from ${flag})`);
        seen.add(id);
    };
    const choicesById = new Map();
    for (const raw of inline.choicesFor) {
        const { id, items } = splitList(raw, "--choices");
        if (choicesById.has(id))
            throw new Error(`duplicate --choices for: ${id}`);
    ...
    for (const raw of inline.choices) {
        ...
        if (!options)
            throw new Error(`--choice ${id} needs a matching --choices ${id}=a,b,...`);
        const criteria = {};
        for (const option of options)
            criteria[option] = option;
        const question = { type: "choice", instructions: value, criteria };
        out.push({ id, kind: "choice", question: question });
    }
    for (const raw of inline.scores) {
        ...
        if (!levels)
            throw new Error(`--score ${id} needs a matching --levels ${id}=low,...,high`);
        if (levels.length < 2)
            throw new Error(`--levels ${id} needs at least two levels`);
        const question = { type: "score", instructions: value, criteria: levels };
        out.push({ id, kind: "score", question: question });
    }
    for (const id of choicesById.keys()) {
        if (!seen.has(id))
            throw new Error(`--choices ${id} has no matching --choice question`);
    }
    for (const id of levelsById.keys()) {
        if (!seen.has(id))
            throw new Error(`--levels ${id} has no matching --score question`);
    }
    return out;
}
export async function parseFile(cwd, path, taken) {
    ...
    try {
        ...
        throw new Error(`cannot read questions file ${path}: ${error.message}`);
    }
    ...
    for (const [id, entry] of Object.entries(parsed.data)) {
        if (taken.has(id))
            throw new Error(`duplicate question id: ${id} (questions file vs flags)`);
        taken.add(id);
        const type = entry.type;
        if (type === "boolean" || type === "noul") {
            const question = { type: "noul", instructions: entry.instructions };
